...
def _set_recursive(data, keys, value):
    ...
    if isinstance(current, int):
        if not isinstance(data, list):
            raise TypeError(
                "Expected a list at index, found '{}'".format(type(data).__name__)
            )
        while len(data) <= current:
            data.append(None)
        if data[current] is None or (
            keys and not isinstance(data[current], (dict, list))
        ):
        ...
        if not isinstance(data, dict):
            raise TypeError(
                "Expected a dict at key '{}', found '{}'".format(
                    current, type(data).__name__
                )
            )
        if current not in data or (
            keys and not isinstance(data[current], (dict, list))
        ):
            data[current] = _create_structure_for_next_key(keys)
        data[current] = _set_recursive(data[current], keys, value)
